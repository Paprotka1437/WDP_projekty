...
void scanning (int n, vector <pair <int, int>> &points){
        points.resize (n);
        for (int i = 0; i < n; ++i){
                if (!(cin >> points[i].first >> points[i].second)) cerr << "insufficient input \n";
                if (i >= 1) assert (points[i].first > points[i - 1].first);
        }
        return;
}
